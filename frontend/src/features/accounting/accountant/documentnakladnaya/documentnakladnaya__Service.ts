...
const API_URL = '/api/accounting/documentnakladnaya';

const item__add = async (
  dataObject: I_DocumentNakladnaya
): Promise<I_DocumentNakladnaya> => {
...
const item__get_all = async (
  dataObject?: I_DocumentNakladnaya
): Promise<I_ServerResponse<I_DocumentNakladnaya>> => {
  const token = JSON.parse(localStorage.getItem('token')!);
  const config = {
    headers: {
      Authorization: `Bearer ${token}`,
