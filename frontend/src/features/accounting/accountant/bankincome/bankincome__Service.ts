...
const API_URL = '/api/accounting/bankincome';

const item__add = async (dataObject: I_BankIncome): Promise<I_BankIncome> => {
...
const item__update = async (
  dataObject: I_BankIncome
): Promise<I_BankIncome> => {
  const token = JSON.parse(localStorage.getItem('token')!);
  const config = {
    headers: {
      Authorization: `Bearer ${token}`,
...
const item__get_all = async (
  dataObject?: I_BankIncome
): Promise<I_ServerResponse<I_BankIncome>> => {
  const token = JSON.parse(localStorage.getItem('token')!);
  const config = {
    headers: {
      Authorization: `Bearer ${token}`,
