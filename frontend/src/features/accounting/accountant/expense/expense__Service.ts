...
const API_URL = '/api/accounting/expense';

const item__add = async (dataObject: I_Expense): Promise<I_Expense> => {
...
const item__update = async (dataObject: I_Expense): Promise<I_Expense> => {
  const token = JSON.parse(localStorage.getItem('token')!);
  const config = {
    headers: {
      Authorization: `Bearer ${token}`,
...
const item__get_one = async (dataObject: I_Expense): Promise<I_Expense> => {
  const token = JSON.parse(localStorage.getItem('token')!);
  const config = {
    headers: {
      Authorization: `Bearer ${token}`,
...
const item__delete_one = async (dataObject: I_Expense): Promise<I_Expense> => {
  const token = JSON.parse(localStorage.getItem('token')!);
  const config = {
    headers: {
      Authorization: `Bearer ${token}`,
...
const item__get_all = async (
  dataObject?: I_Expense
): Promise<I_ServerResponse<I_Expense>> => {
  const token = JSON.parse(localStorage.getItem('token')!);
  const config = {
    headers: {
      Authorization: `Bearer ${token}`,
