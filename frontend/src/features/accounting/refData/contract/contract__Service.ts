...
const API_URL = '/api/refdata/contract';

const item__add = async (dataObject: I_Contract): Promise<I_Contract> => {
...
const item__update = async (dataObject: I_Contract): Promise<I_Contract> => {
  const token = JSON.parse(localStorage.getItem('token')!);
  const config = {
    headers: {
      Authorization: `Bearer ${token}`,
...
const item__get_one = async (dataObject: I_Contract): Promise<I_Contract> => {
  const token = JSON.parse(localStorage.getItem('token')!);
  const config = {
    headers: {
      Authorization: `Bearer ${token}`,
...
const item__get_all = async (
  dataObject?: I_Contract
): Promise<I_ServerResponse<I_Contract>> => {
  const token = JSON.parse(localStorage.getItem('token')!);
  const config = {
    headers: {
      Authorization: `Bearer ${token}`,
