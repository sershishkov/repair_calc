...
const API_URL = '/api/refdata/group-thirdparty-service';

const item__add = async (
  dataObject: I_GroupThirdPartyService
): Promise<I_GroupThirdPartyService> => {
...
const item__get_all = async (
  dataObject?: I_GroupThirdPartyService
): Promise<I_ServerResponse<I_GroupThirdPartyService>> => {
  const token = JSON.parse(localStorage.getItem('token')!);
  const config = {
    headers: {
      Authorization: `Bearer ${token}`,
