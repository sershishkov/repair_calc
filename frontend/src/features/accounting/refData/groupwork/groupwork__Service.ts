...
const item__add = async (dataObject: I_GroupWork): Promise<I_GroupWork> => {
...
const item__update = async (dataObject: I_GroupWork): Promise<I_GroupWork> => {
  const token = JSON.parse(localStorage.getItem('token')!);
  const config = {
    headers: {
      Authorization: `Bearer ${token}`,
...
const item__get_one = async (dataObject: I_GroupWork): Promise<I_GroupWork> => {
  const token = JSON.parse(localStorage.getItem('token')!);
  const config = {
    headers: {
      Authorization: `Bearer ${token}`,
