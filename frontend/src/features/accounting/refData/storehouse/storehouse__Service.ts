...
const API_URL = '/api/refdata/storehouse';

const item__add = async (dataObject: I_StoreHouse): Promise<I_StoreHouse> => {
...
const item__update = async (
  dataObject: I_StoreHouse
): Promise<I_StoreHouse> => {
  const token = JSON.parse(localStorage.getItem('token')!);
  const config = {
    headers: {
      Authorization: `Bearer ${token}`,
...
const item__get_all = async (
  dataObject?: I_StoreHouse
): Promise<I_ServerResponse<I_StoreHouse>> => {
  const token = JSON.parse(localStorage.getItem('token')!);
  const config = {
    headers: {
      Authorization: `Bearer ${token}`,
