...
function EditServiceWork() {
  ...
  useEffect(() => {
    if (id) {
      dispatch(servicework__get_one({ _id: id }));
    }
  ...
  useLayoutEffect(() => {
    if (item) {
      ...
      const arrToSet_equipmentAndTools = item.equipmentAndTools!.map((item) => {
        return typeof item !== 'string' ? item._id! : item;
      });
      set__groupWork(arrToSet_groupWork);
      set__products(arrToSet_products);
      set__equipmentAndTools(arrToSet_equipmentAndTools);
    }
  }, [item]);
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    dispatch(servicework__update(created__Data));
  };
