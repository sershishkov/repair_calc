...
function EditThirdPartyServ() {
  ...
  useEffect(() => {
    if (id) {
      dispatch(thirdPartyService__get_one({ _id: id }));
    }
  ...
  useLayoutEffect(() => {
    if (item) {
      ...
      const arrToSet_groupThirdPartyService = item.groupThirdPartyService!.map(
        (item) => {
          return typeof item !== 'string' ? item._id! : item;
        }
      );

      set__groupThirdPartyService(arrToSet_groupThirdPartyService);
    }
  }, [item]);
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    dispatch(thirdPartyService__update(created__Data));
  };
...
export default EditThirdPartyServ;
